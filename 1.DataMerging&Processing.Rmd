...
# Updated in December: this file will be read in later codes
...
# Removing building Ids to balance the dataset
```{r Aadit, eval=FALSE, echo=TRUE}
...
# On Nov 28th, Biswadip worked on the dataset after Aadit modified the dataset and shared the final csv file on google drive: "https://drive.google.com/file/d/1LnsriFUWdSw_rZK458_PJLmeq8arfjE-/view"
```



```{r Revanth, eval=FALSE, echo=TRUE}
# Work done by Revanth as on Nov 28
# I am naming the csv file that Biswadip provided as df.
...
# Saving the df to a new csv file called as df-for-ml-pipeline.csv
write.csv(df, "df-for-ml-pipeline.csv", , col.names = TRUE, row.names = FALSE)

# As of now, df is the final dataset that's going to be used in a new Rmd file so that we can work on the Machine Learning Pipeline.
```

## 3.2 model building (temporary version)

```{r Hang simply trying to create a linear model, eval=FALSE, echo=TRUE}
library(caret)
set.seed(222)
