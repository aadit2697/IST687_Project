...
### Work done by Revanth as on Nov 28th and Nov 29th.
...
# )
# 
# cols_to_mode <- setdiff(names(df), c(cols_to_sum, cols_to_mean))
# 
# # Use the columns in the summarize function
# df3 <- df %>%
#   # Create a new column for time period based on hours
#   mutate(
#     time_period = case_when(
#       hour(time) %in% 6:11 ~ "Morning",
#       hour(time) %in% 12:15 ~ "Afternoon",
#       hour(time) %in% 16:19 ~ "Evening",
#       hour(time) %in% c(20, 21, 22, 23, 0, 1, 2, 3, 4, 5) ~ "Night",
#       TRUE ~ NA_character_
#     ),
#     plain_date = as.Date(time)  # New column for plain date
#   ) %>%
#   # Group by plain date, time period
#   group_by(plain_date, time_period) %>%
...
#       cols_to_mode,
#       list(
#         mode = ~ as.character(which.max(table(.)))
#       )
#     ),
#     .groups = 'drop'
#   )
